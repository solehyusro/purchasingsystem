 ...
 * @author michael
 */
@WebServlet(name = "pembelian1Servlet", urlPatterns = {"/pembelian1Servlet"})
public class pembelian1Servlet extends HttpServlet {
                    ...
                    case "Simpan":
                        ...
                        if (pembelian.getKode_pembelian().equals("") || pembelian.getTanggal().equals("") || pembelian.getKode_barang().equals("") || pembelian.getKode_supplier().equals("") || pembelian.getSatuan().equals("") || pembelian.getHarga_beli().equals("") || pembelian.getQty().equals("") || pembelian.getSubtotal().equals("")){
                            out.println("<script>alert('Gagal... masih ada data yang belum terisi, Silahkan Ulangi!!!')"
                                    + "</script><meta http-equiv='refresh' content='1;beranda1.jsp?hal=pembelian'/>");
                        } else {
                            ...
                            pstmt2 = koneksi.prepareStatement("UPDATE barang SET stok_barang = stok_barang + ? WHERE kode_barang=?");
                            pstmt2.setString(1, pembelian.getQty());
                            pstmt2.setString(2, pembelian.getKode_barang());

                            result = pstmt2.executeUpdate();
                            if (result > 0) {
                                out.println("<script> "
                                        + "alert('Data telah ditambahkan');"
                                        + "document.location='beranda1.jsp?hal=pembelian';"
                                        + " </script>");
                            }
                        }
                        break;
